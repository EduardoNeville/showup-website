# Creating a new challenge

## Terms and services

Only if first time on the app

## Ai conversation

Have a conversation with Ai to decide on the best challenge

the user should go ahead and start.

## Definitions

1. Define the challenge (What)
   i. Behavioural challenges are better
   ii. Classic definition that can be internalised by user and used to create
   prompts etc...
2. Resolution (How)
   i. How will the challenge be confirmed by a friend?
   ii. This helps the users understand what he must do to properly do the
   challenge
3. Deposit (Who)
   i. You can either deposit to us or to a friend and link their challenges.
   ii. We recommend this in order to have that extra level of commitment
4. Notifications (Consistency)
   i. This should help make the user understand that this is something help
   will be living with
5. Activity rate (When)
   i. What is the rate the activity will be done.
   ii. This shows how serious the user will be

## Sharing

Sharing is an important part of the challenge. It provides accountability and strengthens your bond with your friends. You have to share the challenge to be able to start it.
